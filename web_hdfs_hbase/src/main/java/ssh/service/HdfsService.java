...
public class HdfsService {
	...
	public boolean checkExist(String folder) throws IllegalArgumentException,
			IOException {
		boolean flag = HUtils.getFs().exists(new Path(folder));
		if (!flag)
	...
	public boolean isDir(String dir) throws IllegalArgumentException,
			IOException {
		boolean flag = HUtils.getFs().isDirectory(new Path(dir));
		return flag;
	}

	public boolean upload(String src, String des) throws Exception {
		try {
			HUtils.getFs().copyFromLocalFile(new Path(src), new Path(des));
	...
	public boolean deleteFile(String fileName) throws Exception {
		boolean flag = false;
		try {
			flag = HUtils.getFs().delete(new Path(fileName), false);
		} catch (IllegalArgumentException | IOException e) {
	...
	public boolean download(String fileName, String localFile) throws Exception {
		boolean flag = true;
		try {
			HUtils.getFs().copyToLocalFile(new Path(fileName),
					new Path(localFile));
		} catch (Exception e) {
			e.printStackTrace();
	...
	public String read(String fileName, String textSeq, int records) throws Exception {
		...
		try{
			if("text".equals(textSeq)){
				data = HUtils.readText(fileName, records);
			}else if("seq".equals(textSeq)){
				data = HUtils.readSeq(fileName, records);
			}else{
